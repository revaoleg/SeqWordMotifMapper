...
class StatPlotReport:
    ...
    def get(self, report="stat report", motif_setting="<unidentified>"):
        return self.get_report(report, motif_setting)
    ...
    def stat_report(self, motif_setting="<unidentified>"):
        ...
        for key in ['coding', 'promoter', 'noncoding']:
            if self.genestat[key].title:
                ...
                report.append(
                    f"\t\tZ-score: "
                    f"{f'{self.genestat[key].Z_score:.2f}' if self.genestat[key].Z_score is not None else 'NA'} ± "
                    f"{f'{self.genestat[key].std_error:.2f}' if self.genestat[key].std_error is not None else 'NA'} "
                    f"with p-value = "
                    f"{f'{self.genestat[key].p_value:.3f}' if self.genestat[key].p_value is not None else 'NA'}\n"
                    f"\t\t\texpected / observed: "
                    f"{round(self.genestat[key].expected_number)} / {self.genestat[key].observed_number}\n"
            )
        ...
        if len(self.contigs):
            ...
            for contig in self.contigs:
                ...
                p = f"{abs(contig.p_value):.3f}" if contig.p_value is not None else "na"
                
                report.append(
                    f"\t\t[{contig.start}..{contig.end}] "
                    f"Z-score: {contig.Z_score:.2f} \u00B1 {contig.std_error:.2f} "
                    f"with p-value = {p}\n"
                    f"\t\t\texpected / observed: "
                    f"{round(contig.expected_number)} / {contig.observed_number}\n")

            if self.contigs.para['anova'] is None:
                report.append("\tThe hypotheses of biased or uniform could not be checked;\n")
            elif self.contigs.para['anova'] < 0 and self.contigs.para['anova'] > -0.05:
                report.append(f"\tThe hypothesis of uniform distribution of modified sites across contigs is confirmed with p-value {abs(self.contigs.para['anova'])};\n")
            elif self.contigs.para['anova'] > 0 and self.contigs.para['anova'] <= 0.05:
                report.append(f"\tThe hypothesis of biased distribution of modified sites across contigs is confirmed with p-value {self.contigs.para['anova']};\n")
            else:
                report.append(f"\tThe hypotheses of biased and uniform distribution of modified sites across contigs are rejected with p-value {abs(self.contigs.para['anova'])};\n")
