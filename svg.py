...
class SVG:
    ...
    def __add__(self, oSVG):
        ...
        svg1[0] = f"<svg xmlns=\"http://www.w3.org/2000/svg\" xmlns:xlink=\"http://www.w3.org/1999/xlink\" viewBox=\"0 0 {max_width} {max_height + 50}\">"
        if svg2[-1] != "</svg>":
            svg2.append("</svg>")
        svg = "\n".join(svg1[:-1]) + "\n" + "\n".join(svg2[1:])
        return SVG(svg=svg, width=max_width, top_indend=self.top_indend, title=self.title)
        
    def join(self, oSVG, self_tag = None, self_transform = "", other_tag = None, other_transform = ""):
        # Craete group for first SVG object
        if self_tag:
            svg = self.inlist()
            transform = ""
            if self_transform:
                if self_transform.upper() == "H_CENTER":
                    width1,width2 = [obj.get_width() for obj in [self,oSVG]]
                    max_width = max(width1,width2)
                    transform = f"<g transform=\"translate({(max_width - width1)//2},0)\">"
                else:
                    transform = f"<g transform=\"{self_transform}\">"
            svg.insert(1, f"{transform}<g id=\"{self_tag}\">")
            svg.insert(-1, "</g></g>" if self_transform else "</g>")
            self.SVG = "\n".join(svg)
        # Craete group for second SVG object
        if other_tag:
            svg = oSVG.inlist()
            transform = ""
            if other_transform:
                if other_transform.upper() == "H_CENTER":
                    width1,width2 = [obj.get_width() for obj in [self,oSVG]]
                    max_width = max(width1,width2)
                    transform = f"<g transform=\"translate({(max_width - width2)//2},0)\">"
                else:
                    transform = f"<g transform=\"{other_transform}\">"
            svg.insert(1, f"{transform}<g id=\"{other_tag}\">")
            svg.insert(-1, "</g></g>" if other_transform else "</g>")
            oSVG.SVG = "\n".join(svg)
        return self.__add__(oSVG)
...
class SVG_linear(SVG):
    ...
    def frame(self, loci, X, Y, width, height, title="", histogram="line", rigth_field_indend=50, right_field_top=10, flg_StartEnd=None, 
        ...
        if histogram == "line":
            ...
            for i in range(len(loci_parsed)):
                ...
                # Set bar color
                if central_line > 0:
                    if value < (central_line + minVal) / 2:
                        bar_color = "blue"
                    if value > (central_line + maxVal) / 2:
                        bar_color = "brown"
                else:
                    if value < (maxVal + minVal) / 4:
                        bar_color = "blue"
                    if value > 3 * (maxVal + minVal) / 4:
                        bar_color = "brown"
                ...
                path_data.append((x, y1, y2, bar_width, bar_color, f"{value} [{start}..{end}]"))
                bar_color = "green"
    
            # Construct bar element
            if len(path_data) > 1:
                bars = ""
                for x, y1, y2, bar_width, bar_color, title in path_data:
                    bars += (f"<line x1=\"{x}\" y1=\"{y1}\" x2=\"{x}\" y2=\"{y2}\" fill=\"none\" stroke=\"{bar_color}\" stroke-width=\"{bar_width}\" stroke-linejoin=\"round\">\n" +
    ...
    def set_svg(self, verified=True):
        ...
        self.SVG += (f"<line x1=\"{self.left_indend}\" y1=\"{self.work_height}\" x2=\"{self.left_indend + self.length}\" y2=\"{self.work_height}\" " +
            f"fill=\"none\" stroke=\"grey\" stroke-width=\"{self.chromosome_width - 2}\" stroke-linejoin=\"round\" />\n")
        # Contig lines
        line_width = 3
        line_shift = 4
...
class SVG_circular(SVG):
    ...
    def _format_title(self):
        ...
        if self.info[0] != "":
            direct = [int(v) - 1 for v in self.info[0].split(",")]
    
        if self.info[1] != "":
            reverse = [int(v) - 1 for v in self.info[1].split(",")]
    
        points = sorted(direct + reverse)
        ...
        if points:
            if points[0] in direct:
                title = (
                    self.title[:points[0]]
                    + f"<tspan fill='red' text-decoration='underline'>{self.title[points[0]]}</tspan>"
                )
            else:
                title = (
                    self.title[:points[0]]
                    + f"<tspan fill='blue' font-style='italic'>{self.title[points[0]]}</tspan>"
                )
    
        for i in range(1, len(points)):
            if points[i] in direct:
                title += (
                    self.title[points[i - 1] + 1:points[i]]
                    + f"<tspan fill='red' text-decoration='underline'>{self.title[points[i]]}</tspan>"
                )
            else:
                title += (
                    self.title[points[i - 1] + 1:points[i]]
                    + f"<tspan fill='blue' font-style='italic'>{self.title[points[i]]}</tspan>"
                )
    ...
    def add_task(self, task, task_description, windows):
        ...
        if not task_description["condition"]:
            self.task_lines["circles"] = (
                f"<circle cx=\"{self.center}\" cy=\"{self.center}\" r=\"{mid}\" "
                f"fill=\"white\" stroke=\"grey\" stroke-width=\"0.5\" />\n"
            )
    
        elif task_description["condition"] == "bigger than":
            self.task_lines["circles"] += (
                f"<circle cx=\"{self.center}\" cy=\"{self.center}\" r=\"{mid + band / 2.0}\" "
                f"fill=\"aliceblue\" stroke=\"black\" stroke-width=\"0.5\" />\n"
            )
            self.task_lines["circles"] += self.task_inner_circle(
                task_description["mode"], "white", band, mid, mean, stdev, float(task_description["val1"])
            )
            ...
            self.task_lines["circles"] += self.task_inner_circle(
                task_description["mode"], "aliceblue", band, mid, mean, stdev, float(task_description["val1"])
            )
            ...
            self.task_lines["circles"] += self.task_inner_circle(
                task_description["mode"], "aliceblue", band, mid, mean, stdev,
                float(task_description["val1"]), float(task_description["val2"])
            )
        ...
        for win in wins:
            try:
                first, second = [float(v) for v in win.split("-")]
            except:
                first, second = [float(v) for v in win.split("..")]
            ...
            if VAL is None:
                ...
                self.task_lines["graphs"][task] += (
                    f"<path style=\"stroke:{self.colors[self.added_tasks]};stroke-width:1.0\" "
                    f"d=\"M{x:f},{y:f}"
                )
                continue
            ...
            self.task_lines["graphs"][task] += f"L{x:f},{y:f}"
        ...
        self.task_lines["graphs"][task] += (
            f"<line x1=\"50\" y1=\"{self.width + 25 * self.added_tasks}\" "
            f"x2=\"100\" y2=\"{self.width + 25 * self.added_tasks}\" "
            f"fill=\"none\" stroke=\"{self.colors[self.added_tasks]}\" "
            f"stroke-width=\"1.0\" stroke-linejoin=\"round\" stroke-miterlimit=\"10\" />\n"
        )
    
        self.task_lines["graphs"][task] += (
            f"<text x=\"125\" y=\"{self.width + 3 + 25 * self.added_tasks}\" "
            f"style=\"text-anchor:start;fill:black\">{task}</text>\n"
        )
    ...
    def task_inner_circle(self, mode, fill_color, band, mid, mean, stdev, value1, value2=None):
        if mode == "absolute":
            ...
            line = (
                f"<circle cx=\"{self.center}\" cy=\"{self.center}\" r=\"{r}\" "
                f"fill=\"{fill_color}\" stroke=\"none\" stroke-width=\"0.5\" />\n"
            )
        ...
        if value2 is not None:
            line += "\n" + self.task_inner_circle(
                mode, "white", band, mid, mean, stdev, value2
            )
    ...
    def add_gene_as_bars(self, lb, rb, color="", title="", strand="dir", i=0):
        ...
        if color:
            if color == "grey":
                self.flg_wrongLoci = True
            color = f"fill=\"{color}\" stroke-width=\"3\" stroke=\"{color}\""
    
        if title:
            self.mge_boxes.append(f"<a xlink:title=\"{title}\">")
    
        self.mge_boxes.append(f"<path {color} d=\"M{x:f},{y:f}")
    ...
    def add_gene_as_triangles(self, lb, rb, color="", title="", strand="dir", i=0):
        ...
        if color:
            if color == "grey":
                self.flg_wrongLoci = True
            color = f"fill=\"{color}\" stroke=\"{color}\""
    ...
    def add_gi(self, lb, rb, color="", title=""):
        ...
        if title:
            self.mge_boxes.append(f"<a href=\"\" xlink:title=\"{title}\">")
    ...
    def set_svg(self):
        ...
        self.SVG += (
            f"<circle cx=\"{(self.width-2*self.indend)/2+self.indend}\" "
            f"cy=\"{(self.width-2*self.indend)/2+self.indend}\" "
            f"r=\"{(self.width-2*self.indend)/2}\" "
            f"fill=\"none\" stroke=\"black\" stroke-width=\"1.0\" />\n"
        )
    
        self.SVG += (
            f"<line x1=\"{(self.width-2*self.indend)/2+self.indend}\" "
            f"y1=\"{self.indend-20}\" "
            f"x2=\"{(self.width-2*self.indend)/2+self.indend}\" "
            f"y2=\"{self.indend+20}\" "
            f"fill=\"none\" stroke=\"grey\" stroke-width=\"1.0\" "
            f"stroke-linejoin=\"round\" stroke-miterlimit=\"10\" />\n"
        )
    
        self.SVG += (
            f"<line x1=\"{self.width-self.indend-20}\" "
            f"y1=\"{(self.width-2*self.indend)/2+self.indend}\" "
            f"x2=\"{self.width-self.indend+20}\" "
            f"y2=\"{(self.width-2*self.indend)/2+self.indend}\" "
            f"fill=\"none\" stroke=\"grey\" stroke-width=\"1.0\" "
            f"stroke-linejoin=\"round\" stroke-miterlimit=\"10\" />\n"
        )
        ...
        self.SVG += (
            f"<line x1=\"{(self.width-2*self.indend)/2+self.indend}\" "
            f"y1=\"{self.width-self.indend-20}\" "
            f"x2=\"{(self.width-2*self.indend)/2+self.indend}\" "
            f"y2=\"{self.width-self.indend+20}\" "
            f"fill=\"none\" stroke=\"grey\" stroke-width=\"1.0\" "
            f"stroke-linejoin=\"round\" stroke-miterlimit=\"10\" />\n"
        )
    
        self.SVG += (
            f"<line x1=\"{self.indend-20}\" "
            f"y1=\"{(self.width-2*self.indend)/2+self.indend}\" "
            f"x2=\"{self.indend+20}\" "
            f"y2=\"{(self.width-2*self.indend)/2+self.indend}\" "
            f"fill=\"none\" stroke=\"grey\" stroke-width=\"1.0\" "
        ...
        # Add graph title
        self.SVG += (
            f"<text x=\"{self.width+self.indend}\" "
            f"y=\"{self.small_font_size}\" "
            f"style=\"text-anchor:end\" font-size=\"15\">{self.seqname}</text>"
        )
    
        self.SVG += (
            f"<text x=\"{(self.width-2*self.indend)/2+self.indend+10}\" "
            f"y=\"{self.indend-10}\" "
            f"style=\"text-anchor:start\">"
            f"{tools.format_numeric_string(self.seqlength)}</text>"
        )
    
        self.SVG += (
            f"<text x=\"{self.width-self.indend+10}\" "
            f"y=\"{(self.width-2*self.indend)/2+self.indend-10}\" "
            f"style=\"text-anchor:start\">"
            f"{tools.format_number(self.seqlength/4.0,2,-6)} Mbp</text>"
        )
    
        self.SVG += (
            f"<text x=\"{(self.width-2*self.indend)/2+self.indend+10}\" "
            f"y=\"{self.width-self.indend+15}\" "
            f"style=\"text-anchor:start\">"
            f"{tools.format_number(self.seqlength/2.0,2,-6)} Mbp</text>"
        )
    
        self.SVG += (
            f"<text x=\"{self.indend-10}\" "
            f"y=\"{(self.width-2*self.indend)/2+self.indend-10}\" "
            f"style=\"text-anchor:end\">"
            f"{tools.format_number(3.0*self.seqlength/4.0,2,-6)} Mbp</text>"
        )
        ...
        while dot < self.seqlength:
            ...
            self.SVG += (
                f"<circle "
                f"cx=\"{self.center+self.r*math.cos(a):f}\" "
                f"cy=\"{self.center-self.r*math.sin(a):f}\" "
                f"r=\"{s:f}\" "
                f"fill=\"{fill_color}\" "
                f"stroke=\"black\" stroke-width=\"1.0\" />\n"
            )
        ...
        if len(self.info) > 2:
            self.SVG += (
                ...
                f"font-size=\"{self.main_font_size}\" "
                f"font-weight=\"bold\">{self.info[-1]}</text>\n"
            )
        ...
        self.SVG += (
            ...
            f"font-weight=\"bold\">{self._format_title()}</text>\n"
        )
    
        self.SVG += (
            f"<line x1=\"50\" y1=\"{self.width-25}\" "
            f"x2=\"100\" y2=\"{self.width-25}\" "
        ...
        self.SVG += (
            f"<text x=\"125\" y=\"{self.width-20}\" "
            f"style=\"text-anchor:start;fill:black\">Contigs</text>\n"
        )
    
        if self.flg_wrongLoci:
            self.SVG += (
                "<path style=\"fill:grey;stroke:black\" "
                "d=\"M350,600L375,600L375,615L350,615Z\" />\n"
            )
            self.SVG += (
                f"<text x=\"405\" y=\"615\" "
                f"style=\"text-anchor:start;fill:black;"
                f"font-size:{self.main_font_size}\">"
                f"Falsely selected rrn operons;</text>\n"
            )
    ...
    def format_contigs(self, contig_statistics):    # [{title, start, end, z-score : (score, err, p, exp, found)}...
        is_list = False
        if isinstance(self.SVG, list):
            self.SVG = "\n".join(self.SVG)
            is_list = True
            
        for contig_data in contig_statistics:
            p = float(contig_data['Z-score'][2])
            if p <= 0.05:
                z_score = float(contig_data['Z-score'][0])
                color = "brown"
                if z_score < 0:
                    color = "blue"
                original_line = f"title=\"[{contig_data['start']}-{contig_data['end']}]\"><path fill=\"green\" stroke-width=\"3\" stroke=\"green\""
                line_length = len(original_line)
                l = self.SVG.find(original_line)
                if l > -1:
                    new_line = f"title=\"[{contig_data['start']}-{contig_data['end']}]\"><path fill=\"{color}\" stroke-width=\"3\" stroke=\"{color}\""
                    self.SVG = self.SVG[:l] + new_line + self.SVG[l + line_length :]
                    
        if is_list:
            self.SVG = self.inlist()
