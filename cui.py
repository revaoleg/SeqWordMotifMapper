...
class Validator:
    def __init__(self):
        ...
        self.template_options = {
               ...
               "--dotplot": {
                   ...
                   "-dpc":21,           # modification quality score cutoff
    ...
    def validate_field(self, v, field, para='', options={}):
        ...
        if field == "-g":
            return self.validate_input_files(os.path.join(para, v),["GBK","GB","GBF","FASTA","FA","FST","FNA"])
        if field == "-m":
        ...
        if field == "-wl":
            return self.validate_posnumber(field,v,lower_cutoff=1000, upper_cutoff=50000)
        if field == "-ws":
    ...
    def validate_all(self, options, echo=True):
        ...
        for p in self.get_option_list(plots = plot_setting):
            # Do not check input files as they set by the master program
            if p in ('-i', '-g'):
                continue
            if p not in ('-i','-m'):
    ...
    def validate_input_files(self, generic_fname, extensions, flg_allow_empty=False):
        if flg_allow_empty:
            return True
        if (os.path.exists(generic_fname) and 
            os.path.isfile(generic_fname) and 
            (not extensions or generic_fname[generic_fname.rfind(".")+1:].upper() in extensions)):
            return True
    ...
    def validate_motifs(self,motifs):   # string of motifs is like "GATC,2,-2; -AGNCT,1,-1", where numbers identify modified nucleotides on both strands
        if not motifs:
            return True
        pattern = r"\b[AUGCTRYKMSWBDHVN]+\b"
